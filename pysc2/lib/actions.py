...
def select_rect(action, select_add, screen_rect):
...
class ArgumentType(collections.namedtuple(
  ...
  @classmethod
  def rectangle(cls):  # No range because it's unknown at this time.
    """Create an ArgumentType that is represented by a point.Rect."""
    pt = lambda a: point.Point(*a).floor()
    return cls(-1, "<none>", (0,) * 4,
               lambda a: point.Rect(pt(a[0:2]), pt(a[2:4])))
...
class Arguments(collections.namedtuple("Arguments", [
    "screen", "minimap", "screen_rect", "queued", "control_group_act",
  ...
  Attributes:
    ...
    screen_rect: A rectangle on the screen.
...
TYPES = Arguments.types(
    ...
    screen_rect=ArgumentType.rectangle(),
...
FUNCTION_TYPES = {
    ...
    select_rect: [TYPES.select_add, TYPES.screen_rect],
